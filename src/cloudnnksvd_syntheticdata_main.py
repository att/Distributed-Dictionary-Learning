...
for i in range(NodeN):
    D_local[:,:,:,i] = dict_learning_nnksvd(Y[i], D_init, sp, max_iter, coefupdateiter)

TestLocalErrorMat = np.zeros([max_iter+1, NodeN])
for i in range(max_iter+1):
    print(i)
    for j in range(NodeN):
        temptheta2 = sparse_encode_nnmp(Ytest, D_local[:,:,i,j], sp, coefupdateiter)
        TestLocalErrorMat[i,j] = np.mean( ((Ytest - np.dot(D_local[:,:,i,j], temptheta2)) ** 2).sum(axis=0) )


""" centralized dictionary learning """   
D_central = dict_learning_nnksvd(Y_central, D_init, sp, max_iter, coefupdateiter)
TestCentralErrorMat = np.zeros(max_iter+1)
for i in range(max_iter+1):
    temptheta2 = sparse_encode_nnmp(Ytest, D_central[:,:,i], sp, coefupdateiter)
